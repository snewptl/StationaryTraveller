...
[博客中的题解（推荐）](http://snewptl.com/2024/02/07/Codeforces-Round921Div2/)
...
遇到这种题目直接打表。先限定每个元素的范围为 $[-5, 5]$ ，找出 n 为 1...8 的所有可能序列，发现 ${-4, -3, 3, -1, 1, 4}$ 多次在不同地方重复地部分或全部出现。研究发现这一段数字每依次增加两个都会使得新序列依然满足条件。

所以找到$n = 2$ 和 $n = 5$ 中末尾两位以${-4, -3}, {3, -1}, {1, 4}$这三种数组组合结尾的序列，以此为基础。如果有更大的 n ，对 n 按照奇偶进行讨论，然后从这两种基础序列扩展出来即可。
