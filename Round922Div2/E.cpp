...
int setXEqualTo(int pos) {
    while (askWith(pos)) {
        continue;
    }
}
int bruteForce(std::vector<int> vec, int nearestKownPosition) {
    setXEqualTo(nearestKownPosition);
    int last = vec[nearestKownPosition];
    for (auto it : vec) {
        int diff = 0;
        a[it] = last;
        do {
...
/*
int shuffleSelect(vector<int>)
void solve(vector<int>)
int getAccurateNumber(int pos)
*/
