...
有两种操作，一种操作是在数轴中一个地方增加一个harbour；另一种操作是查询一段区间内所有飞船气飞花销的总和。
...
这道题目还有一个极其恶心的地方，就是其中的一个计算步骤中出现了 $\frac{1}{2^n-1}$ ，如果对 $2^n-1$ 求逆元的话，根据费马小定理，这个式子会在 $n = 999999893$ 的时候变成 $0$，导致结果出错。所以得对这种情况特判断，换一下计算方式。不得不说,如果在比赛的途中遇到这个问题，真的很让人心态爆炸。
